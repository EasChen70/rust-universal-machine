...
pub fn rum(first: Vec<u32>) {
    ...
    let mut identifier: Vec<usize> = Vec::new(); // Changed type to Vec<usize>
    ...
    while pc < memory[0].len() {
        ...
        if hold.0 > 13 {
            ...
            register[hold.1 as usize] = simple_instructions(hold.0, register[hold.2 as usize], register[hold.3 as usize]);
            pc += 1;
        } else if hold.0 == 7 {
            process::exit(0);
        } else if hold.0 == 8 {
